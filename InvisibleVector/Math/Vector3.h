...
/// <summary>
/// [EN] This struct is contained coordinate "x, y, z". [JP] x,y,zの座標を内包した構造体です。
/// </summary>
...
struct Vector3Base
{
	...
	/// <param name="z">[EN] Coordinate z. [JP] Z座標</param>
	Vector3Base(ValueType x, ValueType y, ValueType z) : x{ x }, y{ y }, z{ z } {}

	/// <summary>
	/// [EN] Calculate Dot. 
	/// [JP] 内積計算を行う
	/// </summary>
	/// <param name="vec01">[EN] Vector3 [JP] Vector3クラスを入れる</param>
	/// <param name="vec02">[EN] Vector3 [JP] Vector3クラスを入れる</param>
	/// <returns></returns>
	static ValueType Dot(const Vector3Base& vec01, const Vector3Base& vec02)
	{
		return vec01.x * vec02.x + vec01.y * vec02.y + vec01.z * vec02.z;
	}
	...
	/// <param name="vec01">[EN] Vector3 [JP] Vector3クラスを入れる</param>
	/// <param name="vec02">[EN] Vector3 [JP] Vector3クラスを入れる</param>
	/// <returns>[EN] Returns the vector perpendicular to two vectors
	///	         [JP] 二つのベクトルに対して垂直なベクトルを返す</returns>
	static Vector3Base Cross(const Vector3Base& vec01, const Vector3Base& vec02)
	{
		return Vector3Base{
...
/// <returns>[EN] Vector after subtract [JP] 減算後のベクトル</returns>
	...
	Vector3Base operator *(const ValueType& value) const
	{
		return Vector3Base{
			x * value,
			y * value,
	...
	/// <param name="value">[EN] scalar [JP] スカラー値</param>
	/// <returns>[EN] Vector after scalar multiplication [JP] スカラー乗算後のベクトル</returns>
	friend Vector3Base operator *(const ValueType value, const Vector3Base& other)
	{
		return other * value;
	}
